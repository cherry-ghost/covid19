...
```{r data_import, message=FALSE, warning=FALSE, tidy=TRUE, echo=TRUE}
input_csv <- paste0("./webdata/time_series_19-covid-Confirmed.csv")
input_url <- "https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_19-covid-Confirmed.csv"
...
#sir_start_date <- "2020-02-26"
sir_end_date <- "2020-03-16"
#sir_end_date <- "2020-03-17"

Infected <- confirmed_cases_jh %>%
...
fitted_cumulative_incidence <- fitted_cumulative_incidence %>% 
  ...
  left_join(confirmed_cases_jh, by = "Date") %>% ## NEU
...
fitted_cumulative_incidence %>%
  ...
  geom_line(aes(y=S, colour="black")) +
  geom_line(aes(y=R, colour="green")) +
  geom_line(aes(y=I*0.05, colour="red4")) +
  geom_point(aes(y=cumulative_cases, colour="orange")) +
  # Plätze in der Intensivpflege
  geom_hline(yintercept=1500, linetype = "dotdash", colour = "gray40") +
  annotate(geom='text', x=ymd("2020-04-25"),y=600, colour = "gray40",
  ...
  scale_colour_manual(name = '', 
                      ...
                      labels = c('Susceptible', 'Recovered', 'Observed incidence',
                                 'Infectious',
                                 'Infectious - Higher Risk (5%)'
                                 )) +
  theme_bw()
## To DO:
## - Bundesweite Massnahmen auf Zeitachse eintragen: 1. Empfehlung BR, Lockdown, evt Ausgangssperre, ...
## - Risikogruppe und nicht Risikogruppe anhand der "Infectious" Kurve separieren
## - Allenfalls Schweizerbevölkerung eintragen (N)
## - Infectious in Infectious ohne Massnahmen umbenennen
```
...
fitted_cumulative_incidence_as <- fitted_cumulative_incidence_as %>% 
  mutate(Date = ymd(sir_start_date) + days(t - 1)) %>% 
  left_join(confirmed_cases_jh, by = "Date")
